...
<body class="min-h-screen bg-black text-white">
        ...
        <div class="max-w-7xl mx-auto px-10 py-4 flex flex-wrap items-center justify-between">
            ...
            <div class="hidden lg:flex items-center justify-center gap-6">
                <a href="{{ route('landingpage') }}#movies"
                    class="text-yellow-500 hover:text-yellow-600 hover:border-b font-medium transition-all pb-3">Movies</a>
                <a href="{{ route('landingpage') }}#movies"
                    class="text-yellow-500 hover:text-yellow-600 hover:border-b pb-3 font-medium transition">Schedule</a>
                <a href="{{ route('movies.favorites') }}"
                    class="text-yellow-300 border-b-2 border-yellow-400 pb-3 font-medium transition">Favourites</a>
                @if (Auth::check())
                    <a href="{{ route('movie.ticket') }}">
                        <button
                            class="cursor-pointer relative flex gap-1 hover:bg-gradient-to-r from-yellow-500 to-yellow-700 hover:text-black bg-black text-yellow-600 border border-yellow-400 text-black px-6 py-2 rounded-full font-semibold shadow-lg hover:shadow-xl transition transform hover:scale-105">
                            Your Ticket
                            @if ($totalBookings ?? false)
                                <div
                                    class="w-5 h-5 top-0 right-0 bg-yellow-600 absolute flex items-center justify-center rounded-full">
                    ...
                    @if (auth()->user()->role == 'admin')
                        <a href="/dashboard"
                            class="cursor-pointer hover:bg-gradient-to-r from-yellow-500 to-yellow-700 hover:text-black bg-black text-yellow-600 border border-yellow-400 text-black px-6 py-2 rounded-full font-semibold shadow-lg hover:shadow-xl transition transform hover:scale-105">
                            Go to Dashboard
                        </a>
                    @endif
                @else
                    <a href="{{ route('login') }}"
                        class="cursor-pointer bg-yellow-600 hover:bg-yellow-700 text-black px-6 py-2 rounded-full font-semibold shadow-lg hover:shadow-xl transition transform hover:scale-105">
                        Login
                    </a>
                @endif
                <div>
                    ...
                    @foreach ($favorites as $fav)
                        <div class="movie-card rounded-2xl shadow-xs overflow-hidden transition-all duration-500 group"
                            ...
                            <div class="relative h-72 overflow-hidden">
                                <img src="{{ asset('images/' . $fav->movie->image) }}" alt="{{ $fav->movie->name }}"
                                ...
                                <div
                                    class="absolute bottom-0 left-4 right-4 text-yellow-400 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                            ...
                            <div class="p-6">
                                <h3
                                    class="text-xl font-bold text-yellow-400 mb-3 group-hover:text-yellow-300 transition">
                                    {{ $fav->movie->name }}</h3>
                                <p class="text-gray-300 text-sm mb-6 h-17 line-clamp-3 leading-relaxed">
                                    {{ $fav->movie->description }}</p>

                                <div class="flex flex-wrap gap-3 justify-between items-center">
                                    <form action="{{ route('movie.details', $fav->movie->id) }}" method="GET"
                                        class="flex-1 min-w-0">
                                        @csrf
                                        <button type="submit"
                                            class="w-full cursor-pointer bg-yellow-500 hover:bg-yellow-600 text-black py-3 px-2 rounded-xl font-semibold shadow-lg hover:shadow-xl transition transform hover:scale-105">
                                            Details
                                        </button>
                                    </form>
                                    <form action="{{ route('movies.book', $fav->movie->id) }}" method="GET"
                                        class="flex-1 min-w-0">
                                        @csrf
                                        <button type="submit"
                                            class="flex gap-3 items-center cursor-pointer justify-center w-full bg-yellow-500 hover:bg-yellow-600 text-black py-3 px-2 rounded-xl font-semibold shadow-lg hover:shadow-xl transition transform hover:scale-105">
                                            Book Now
    ...
    <section class="flex items-center justify-center">
        <a href="/"
            class="bg-gradient-to-r from-yellow-400 to-yellow-600 hover:from-yellow-500 hover:to-yellow-700 text-black font-semibold py-3 px-8 rounded-lg transition transform hover:scale-105 inline-flex items-center">
